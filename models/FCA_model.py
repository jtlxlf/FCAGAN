...
class FCAModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):

        parser.set_defaults(norm='batch', netG='FCA', dataset_mode='font')
        
        if is_train:
            parser.set_defaults(batch_size=256, pool_size=0, gan_mode='hinge',gan_modes='hinge2' ,netD='basic_64',netD_s ='basic_64_c')
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:  # define discriminators; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
            if self.dis_2:
                self.netD_content = networks.define_D(2, opt.ndf, opt.netD_s,opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids, use_spectral_norm=opt.use_spectral_norm)
                self.netD_style = networks.define_D(self.style_channel+1, opt.ndf, opt.netD_s, opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids, use_spectral_norm=opt.use_spectral_norm)
            else:
        ...
        if self.isTrain:
            ...
            self.criterionGANs = networks.GANLoss('hinge2').to(self.device)
    ...
    def compute_gan_loss_D(self, real_images, fake_images ,netD):
        ...
        return loss_D
    def compute_gan_loss_Ds(self, real_images, fake_images ,netD):
        # Fake
        fake = torch.cat(fake_images, 1)
        pred_fake = netD(fake.detach())
        loss_D_fake = self.criterionGANs(pred_fake , False)
        # Real
        real = torch.cat(real_images, 1)
        pred_real =  netD(real)
        loss_D_real = self.criterionGANs(pred_real, True)
        # combine loss
        loss_D = (loss_D_fake + loss_D_real) * 0.5
        return loss_D
    def compute_gan_loss_G(self, fake_images, netD):
        ...
        return loss_G_GAN
    def compute_gan_loss_Gs(self, fake_images, netD):
        fake = torch.cat(fake_images, 1)
        pred_fake = netD(fake)
        loss_G_GAN = self.criterionGANs(pred_fake, True, True)
        return loss_G_GAN
    def backward_D(self):
        """Calculate GAN loss for the discriminator"""
        if self.dis_2:
            self.loss_D_content = self.compute_gan_loss_Ds([self.content_images, self.gt_images],  [self.content_images, self.generated_images], self.netD_content)
            self.loss_D_style = self.compute_gan_loss_Ds([self.style_images, self.gt_images], [self.style_images, self.generated_images], self.netD_style)
    ...
    def backward_G(self):
        ...
        if self.dis_2:
            self.loss_G_content = self.compute_gan_loss_Gs([self.content_images, self.generated_images], self.netD_content)
            self.loss_G_style = self.compute_gan_loss_Gs([self.style_images, self.generated_images], self.netD_style)
